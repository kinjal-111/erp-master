...
class Sales{
    ...
    public function createInvoiceAndAddInSales($customer_id){
        try{
...
BUTTONS;
    ...
    public function getSalesByID($id, $fetchMode = PDO::FETCH_OBJ)
    {
        $query = "SELECT * FROM {$this->table} WHERE id = {$id} AND deleted = 0";
        $result = $this->database->raw($query,$fetchMode);
        return $result;
    } 
    public function getSalesByInvoiceID($invoice_id, $fetchMode = PDO::FETCH_OBJ)
    {
        $query = "SELECT * FROM {$this->table} WHERE invoice_id = {$invoice_id} AND deleted = 0";
        $result = $this->database->raw($query,$fetchMode);
        return $result;
    }
