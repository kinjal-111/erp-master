...
class Database{
    ...
    public function existsCurrentRecord($table,$data,$tableName, $table_id)
    {
        //$data['name'=>'HT'];
        //Util::dd($data);
        $field = array_keys($data)[0];  
        //Util::dd($field);  
        $result = $this->readData($tableName,[],"{$field} = '{$data[$field]}' and deleted = 0 and id != {$table_id}",PDO::FETCH_ASSOC);
        // Util::dd($result);  
        if(count($result) > 0)
        {
            return true;
