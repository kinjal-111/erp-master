...
class Customer
{
    ...
    public function getCustomerByEmail($email_id, $fetchMode= PDO::FETCH_OBJ){
        $query = "SELECT * FROM {$this->table} WHERE email_id = '{$email_id}' AND deleted = 0";
        $result = $this->database->raw($query,$fetchMode);
        return $result;
    }
...
BUTTONS;
    ...
    {   
        ...
        $this->validateEditData($data_to_be_updated,$data['customer_id']);
        if(!$this->validator->fails())
        {
            try{
                $this->database->beginTransaction();
